 ...
 * yet expose the `/super-admin/*` surface, so these are hand-written. TODO: replace
...
/** `data` of step 1 (`POST /public/super-admin/auth/login`) — the OTP was emailed. */
export interface OtpChallenge {
	status: 'OTP_SENT';
}

/** `GET /super-admin/me` — the authenticated operator's profile (console bootstrap). */
