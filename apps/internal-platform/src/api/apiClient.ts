 ...
 * on app boot and as the super admin client's 401 hook (single-flight is handled
...
export async function runRefresh(): Promise<boolean> {
	...
	try {
		const result = await publicApi.post<AuthResult>('/super-admin/auth/refresh', {
			refreshToken,
		});
...
/**
 * Authenticated platform surface (`/api/v1/super-admin/*`). Attaches the in-memory
