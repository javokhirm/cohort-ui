...
export const handlers = [
	http.post(`${BASE}/public/super-admin/auth/login`, async ({ request }) => {
	...
	http.post(`${BASE}/public/super-admin/auth/verify-otp`, async ({ request }) => {
	...
	http.get(`${BASE}/super-admin/me`, ({ request }) => {
	...
	http.get(`${BASE}/super-admin/plans`, () =>
		okPaged(MOCK_PLANS, 1, 20, MOCK_PLANS.length),
	),

	http.post(`${BASE}/super-admin/plans`, async ({ request }) => {
	...
	http.patch(`${BASE}/super-admin/plans/:id`, async ({ params, request }) => {
	...
	http.get(`${BASE}/super-admin/subscriptions`, () =>
		okPaged(MOCK_SUBSCRIPTIONS, 1, 20, MOCK_SUBSCRIPTIONS.length),
	),

	http.get(`${BASE}/super-admin/subscriptions/analytics`, () =>
	...
	http.get(`${BASE}/super-admin/users`, () =>
		okPaged(MOCK_USERS, 1, 20, MOCK_USERS.length),
	),

	http.get(`${BASE}/super-admin/users/:id`, ({ params }) => {
	...
	http.post(`${BASE}/super-admin/users/:id/deactivate`, ({ params }) => {
		const user = MOCK_USERS.find((u) => u.id === Number(params['id']));
		if (!user) return fail(404, 'NOT_FOUND', 'User not found.');
		return ok({ id: Number(params['id']), isActive: false, status: 'inactive' });
	}),

	http.post(`${BASE}/super-admin/users/:id/reset-password`, ({ params }) => {
		const user = MOCK_USERS.find((u) => u.id === Number(params['id']));
		if (!user) return fail(404, 'NOT_FOUND', 'User not found.');
		return ok(null);
	}),
	...
	http.get(`${BASE}/super-admin/audit-logs`, () =>
		okPaged(MOCK_AUDIT_LOGS, 1, 20, MOCK_AUDIT_LOGS.length),
	),

	http.get(`${BASE}/super-admin/audit-logs/:id`, ({ params }) => {
	...
	http.get(`${BASE}/super-admin/roles`, () => ok(MOCK_ROLES)),

	http.get(`${BASE}/super-admin/permissions`, () => ok(MOCK_PERMISSIONS)),

	http.patch(
		`${BASE}/super-admin/roles/:role/permissions`,
	...
	http.get(`${BASE}/super-admin/tenants`, ({ request }) => {
		const url = new URL(request.url);
		const status = url.searchParams.get('status');
		const search = url.searchParams.get('search')?.toLowerCase() ?? '';
		const page = Number(url.searchParams.get('page') ?? 1);
	...
	http.get(`${BASE}/super-admin/tenants/summary`, () => ok(MOCK_TENANT_SUMMARY)),

	http.get(`${BASE}/super-admin/tenants/:id`, ({ params }) => {
	...
	http.get(`${BASE}/super-admin/tenants/:id/branches`, ({ params }) => {
		const id = Number(params['id']);
		const tenant = MOCK_TENANT_LIST.find((t) => t.id === id);
		if (!tenant) return fail(404, 'NOT_FOUND', 'Tenant not found.');
	...
	http.get(`${BASE}/super-admin/tenants/:id/stats`, ({ params }) => {
		const id = Number(params['id']);
		const tenant = MOCK_TENANT_LIST.find((t) => t.id === id);
		if (!tenant) return fail(404, 'NOT_FOUND', 'Tenant not found.');
		return ok({
	...
	http.patch(`${BASE}/super-admin/tenants/:id`, async ({ params, request }) => {
		const id = Number(params['id']);
		const tenant = MOCK_TENANT_LIST.find((t) => t.id === id);
		if (!tenant) return fail(404, 'NOT_FOUND', 'Tenant not found.');
		const body = (await request.json()) as Record<string, unknown>;
	...
	http.post(
		`${BASE}/super-admin/tenants/:id/change-plan`,
		async ({ params, request }) => {
			const id = Number(params['id']);
			const tenant = MOCK_TENANT_LIST.find((t) => t.id === id);
			if (!tenant) return fail(404, 'NOT_FOUND', 'Tenant not found.');
	...
	http.post(`${BASE}/super-admin/tenants`, async ({ request }) => {
		const body = (await request.json()) as Record<string, unknown>;
		return ok({
			id: 99,
			name: body['name'],
	...
	http.post(`${BASE}/super-admin/tenants/:id/members`, ({ params }) => {
		const tenant = MOCK_TENANT_LIST.find((t) => t.id === Number(params['id']));
		if (!tenant) return fail(404, 'NOT_FOUND', 'Tenant not found.');
		return ok(null);
	}),

	http.patch(`${BASE}/super-admin/tenants/:id/members/:userId`, ({ params }) => {
		const tenant = MOCK_TENANT_LIST.find((t) => t.id === Number(params['id']));
		if (!tenant) return fail(404, 'NOT_FOUND', 'Tenant not found.');
		return ok(null);
	}),

	http.post(`${BASE}/super-admin/tenants/:id/suspend`, ({ params }) =>
		ok({ id: Number(params['id']), status: 'SUSPENDED' }),
	),

	http.post(`${BASE}/super-admin/tenants/:id/unsuspend`, ({ params }) =>
		ok({ id: Number(params['id']), status: 'ACTIVE' }),
	),

	http.post(`${BASE}/super-admin/tenants/:id/cancel`, ({ params }) =>
...
export const planHandlers = {
	empty: http.get(`${BASE}/super-admin/plans`, () => okPaged([], 1, 20, 0)),
	forbidden: http.get(`${BASE}/super-admin/plans`, () =>
		fail(403, 'FORBIDDEN', 'You do not have permission.'),
	),
	serverError: http.get(`${BASE}/super-admin/plans`, () =>
		fail(500, 'INTERNAL_ERROR', 'Unexpected server error.'),
	),
};

export const subscriptionHandlers = {
	empty: http.get(`${BASE}/super-admin/subscriptions`, () => okPaged([], 1, 20, 0)),
	forbidden: http.get(`${BASE}/super-admin/subscriptions`, () =>
		fail(403, 'FORBIDDEN', 'You do not have permission.'),
	),
	serverError: http.get(`${BASE}/super-admin/subscriptions`, () =>
		fail(500, 'INTERNAL_ERROR', 'Unexpected server error.'),
	),
};

export const userHandlers = {
	empty: http.get(`${BASE}/super-admin/users`, () => okPaged([], 1, 20, 0)),
	forbidden: http.get(`${BASE}/super-admin/users`, () =>
		fail(403, 'FORBIDDEN', 'You do not have permission.'),
	),
	serverError: http.get(`${BASE}/super-admin/users`, () =>
		fail(500, 'INTERNAL_ERROR', 'Unexpected server error.'),
	),
};

export const auditHandlers = {
	empty: http.get(`${BASE}/super-admin/audit-logs`, () => okPaged([], 1, 20, 0)),
	forbidden: http.get(`${BASE}/super-admin/audit-logs`, () =>
		fail(403, 'FORBIDDEN', 'You do not have permission.'),
	),
	serverError: http.get(`${BASE}/super-admin/audit-logs`, () =>
		fail(500, 'INTERNAL_ERROR', 'Unexpected server error.'),
	),
};

export const tenantHandlers = {
	empty: http.get(`${BASE}/super-admin/tenants`, () => okPaged([], 1, 20, 0)),
	forbidden: http.get(`${BASE}/super-admin/tenants`, () =>
		fail(403, 'FORBIDDEN', 'You do not have permission.'),
	),
	serverError: http.get(`${BASE}/super-admin/tenants`, () =>
		fail(500, 'INTERNAL_ERROR', 'Unexpected server error.'),
	),
};

export const dashboardHandlers = {
	forbidden: http.get(`${BASE}/super-admin/dashboard`, () =>
		fail(403, 'FORBIDDEN', 'You do not have permission.'),
	),
	serverError: http.get(`${BASE}/super-admin/dashboard`, () =>
		fail(500, 'INTERNAL_ERROR', 'Unexpected server error.'),
	),
};

export const roleHandlers = {
	empty: http.get(`${BASE}/super-admin/roles`, () => ok([])),
	forbidden: http.get(`${BASE}/super-admin/roles`, () =>
		fail(403, 'FORBIDDEN', 'You do not have permission.'),
	),
	serverError: http.get(`${BASE}/super-admin/roles`, () =>
		fail(500, 'INTERNAL_ERROR', 'Unexpected server error.'),
	),
};
