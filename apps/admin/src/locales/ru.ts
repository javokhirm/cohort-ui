...
export const ru: TranslationsOf<typeof uz> = {
	...
	courses: {
		...
		feePlanEmpty:
			'В этом филиале нет активного тарифного плана — курс создать нельзя.',
		feePlanEmptyCta: 'Создать тарифный план',
		planChangeWarning:
	...
	people: {
		...
		detail: {
			...
			performance: {
				emptyTitle: 'Нет записей о посещаемости',
				emptyDescription:
					'Посещаемость и ежедневные оценки появятся здесь, когда в группах ученика проведут занятия.',
				emptyFiltered: 'Нет занятий, подходящих под эти фильтры',
				...
				attendanceMeta: 'посещено {{attended}} из {{total}} занятий',
				belowPolicy: 'Ниже нормы {{threshold}}%',
				averageMark: 'Средняя оценка за занятие',
				markMeta_one: '{{count}} оценённое занятие',
				markMeta_few: '{{count}} оценённых занятия',
				markMeta_many: '{{count}} оценённых занятий',
				markMeta_other: '{{count}} оценённых занятий',
				unmarkedMeta_one: '{{count}} без оценки',
				unmarkedMeta_few: '{{count}} без оценки',
				unmarkedMeta_many: '{{count}} без оценки',
				unmarkedMeta_other: '{{count}} без оценки',
				sessionHistory: 'История занятий',
				notMarked: 'Без оценки',
				noTopic: 'Тема не указана',
				/** Названия шкал — собираются из `type`/`maxPoints`. */
		...
		guardiansPage: {
			...
			detail: {
				...
				linkedStudentsTitle: 'Привязанные ученики',
				linkStudent: 'Привязать ученика',
				unlink: 'Отвязать',
			...
			linkDialog: {
				...
				description: 'Привяжите этого опекуна к существующему ученику.',
				student: 'Ученик *',
				relation: 'Кем приходится *',
				primaryLabel: 'Основной опекун',
				pickupLabel: 'Может забирать',
				submit: 'Привязать',
				linked: 'Ученик привязан',
				linkFailed: 'Не удалось привязать ученика',
			},
		},
	},
	hr: {
	...
	profile: {
		...
		preferencesCardTitle: 'Настройки',
		preferencesHint:
			'Язык интерфейса сохраняется в вашем аккаунте и применяется на всех устройствах.',
	...
	subscription: {
		...
		banner: {
			message:
				'Ваша подписка истекает {{date}}. Продлите план, чтобы избежать перерыва в работе.',
			renew: 'Продлить',
			dismiss: 'Закрыть',
		},
		renewalPrice: 'Цена продления',
		active: {
			period: 'Период',
			renewsOn: 'Дата продления',
		},
		blocked: {
			title: 'Срок подписки истёк',
			description: 'Продлите план, чтобы продолжить пользоваться аккаунтом.',
			expiredOn: 'Дата окончания',
			currentPlan: 'Текущий план',
			renew: 'Продлить',
			ownerOnly: 'Для продления обратитесь к владельцу центра',
		},
		features: {
			title: 'Возможности плана',
			empty: 'Дополнительных возможностей нет',
	...
	notifications: {
		title: 'Коммуникации',
		description: 'Когда сообщения уходят, что в них сказано и что реально дошло',
		tab: {
			rules: 'Правила',
			templates: 'Шаблоны',
			outbox: 'Журнал',
			settings: 'Настройки',
			live: '{{count}} в очереди',
		},
		console: {
			smsOn: 'SMS включены',
			smsOperational: 'SMS работают',
			smsOff: 'SMS выключены',
			smsBroken: 'SMS не настроены',
			usage: '{{sent}} / {{limit}} сегодня',
			usageUncapped: '{{sent}} сегодня',
			sendMessage: 'Отправить сообщение',
			newBroadcast: 'Новая рассылка',
			brokenTitle: 'SMS включены, но ничего не отправляется',
			brokenBody:
				'Правила продолжают срабатывать, сообщения ставятся в очередь — и каждое будет падать, пока это не исправят.',
		...
		rules: {
			...
			toggleFailed: 'Не удалось изменить состояние правила',
			description:
				'Без правила ничего не отправляется. Правило связывает событие с каналом, получателем и шаблоном.',
			add: 'Добавить правило',
			newRule: 'Новое правило',
			edit: 'Редактировать правило',
			timingBefore: 'За {{days}} дн. до',
			timingOn: 'В этот день',
			timingAfter: 'Через {{days}} дн.',
			empty: 'Правил пока нет. Добавьте первое, чтобы начать отправлять сообщения.',
			loadError: 'Не удалось загрузить правила. Обновите страницу.',
			...
			deleteDescription:
				'Правило «{{name}}» будет удалено. Вместо удаления его можно просто выключить.',
			toggleAria: 'Включить или выключить правило {{name}}',
			scheduledGroup: 'Плановые напоминания',
			eventGroup: 'По событию',
			offsetBefore: 'За {{days}} дн. до срока',
			offsetOn: 'В день срока',
			offsetAfter: 'Через {{days}} дн. после срока',
			column: {
				name: 'Название',
				trigger: 'Событие',
				timing: 'Когда',
			...
			field: {
				...
				nameHint: 'Например: «За 3 дня до срока»',
				trigger: 'Событие',
				triggerHint: 'Нельзя изменить после создания',
				channels: 'Каналы',
				audience: 'Получатели',
				templateCode: 'Код шаблона',
				templateCodeHint:
					'Определяется событием — изменить текст можно на вкладке «Шаблоны»',
				offsetDays: 'Смещение в днях',
				offsetDaysHint:
					'Отрицательное — до срока, 0 — в день срока, положительное — после срока',
				isActive: 'Активно',
				isActiveHint:
					'Выключенное правило сохраняется, но не отправляет сообщения',
			},
			validation: {
				channelsRequired: 'Выберите хотя бы один канал',
				offsetRange: 'Смещение должно быть от -365 до 365',
			},
			variablesHint: 'Переменные, доступные для этого события:',
			catalogHint:
				'Получатели, смещения и переменные берутся из каталога триггеров',
			filter: {
				all: 'Все',
				active: 'Активные',
				paused: 'На паузе',
			...
			kindScheduled: 'ПЛАНОВОЕ',
			kindEvent: 'СОБЫТИЕ',
			scheduledHint: 'Отправляется со смещением в днях относительно опорной даты',
			eventHint: 'Срабатывает в момент регистрации события — без смещения',
			stepsCount: '{{count}} шага в порядке отправки',
			stepCountOne: '1 шаг',
			countLabel: 'Правил: {{count}}',
			eventChip: 'СЕЙЧАС',
			openTemplateAria: 'Открыть шаблон «{{code}}»',
			filterEmpty: 'Нет правил под этот фильтр',
			filterEmptyHint:
				'Вернитесь ко всем правилам или создайте правило для триггера из каталога.',
		},
		templates: {
			title: 'Шаблоны сообщений',
			saveFailed: 'Не удалось сохранить шаблон',
			deleteFailed: 'Не удалось удалить шаблон',
			previewFailed: 'Не удалось выполнить предпросмотр',
			description:
				'По одному шаблону на код, канал и язык. При отправке выбирается язык получателя.',
			add: 'Добавить шаблон',
			edit: 'Редактировать шаблон',
			empty: 'Шаблонов пока нет.',
			loadError: 'Не удалось загрузить шаблоны. Обновите страницу.',
			created: 'Шаблон создан',
			updated: 'Шаблон обновлён',
			deleted: 'Шаблон удалён',
			deleteTitle: 'Удалить шаблон',
			deleteDescription:
				'Шаблон {{code}} ({{channel}} / {{locale}}) будет удалён. Отправленные сообщения не изменятся.',
			column: {
				code: 'Код',
				channel: 'Канал',
				locale: 'Язык',
			...
			field: {
				...
				locale: 'Язык',
				subject: 'Тема',
				subjectHint: 'Только для email',
				body: 'Текст сообщения',
				bodyHint: 'Нажмите на переменную, чтобы вставить её',
				isActive: 'Активен',
			},
			insertVariable: 'Вставить {{name}}',
			new: 'Новый шаблон',
			searchPlaceholder: 'Фильтр по коду, каналу, языку',
			selectPrompt: 'Выберите шаблон',
			selectPromptHint:
				'Выберите сообщение из списка, чтобы отредактировать текст и посмотреть превью.',
			editDetails: 'Изменить детали',
			revert: 'Отменить',
			saveButton: 'Сохранить шаблон',
			usedBy: 'Используется в {{count}} правилах',
			usedByNone: 'Не используется ни в одном правиле',
			lastEdited: 'изменён {{date}}',
			variablesTitle: 'Переменные этого шаблона',
			unknownVars:
				'{{vars}} — триггер их не предоставляет, поэтому у каждого получателя они будут пустыми.',
			livePreview: 'Живое превью',
			previewFrom: 'от {{sender}}',
			previewEmpty: 'Введите текст — превью появится здесь.',
			segmentsCounted: 'считается по готовому сообщению, а не по исходному тексту',
			charCount:
				'{{chars}}/{{perSegment}} симв. · {{encoding}} · {{segments}} сегм.',
			sourceDefault: 'По умолчанию',
			sourceCustom: 'Изменён',
			revertTitle: 'Вернуть по умолчанию',
			revertDescription:
				'Изменённый шаблон {{code}} ({{channel}} / {{locale}}) будет удалён, снова будет использоваться шаблон по умолчанию.',
			reverted: 'Возвращён шаблон по умолчанию',
			filterEmpty: 'Нет шаблонов по этому фильтру',
			noActive: 'Нет шаблонов с включённым правилом',
			noActiveHint: 'Включите правило, чтобы настроить его сообщения.',
		},
		moderation: {
			title: 'Одобрение SMS-оператора',
			status: {
			...
			hint: {
				PENDING:
					'Не удалось отправить текст оператору. Он будет отправлен повторно при следующем сохранении шаблона.',
				MODERATION:
					'Текст отправлен оператору и ждёт проверки. До одобрения SMS с этим текстом не доставляются.',
				APPROVED: 'Оператор одобрил текст — SMS по этому шаблону доставляются.',
				REJECTED:
					'Оператор отклонил текст. Повторная отправка того же текста не поможет — измените текст и сохраните.',
			},
			submittedAs: 'В каком виде отправлено',
			errorLabel: 'Ошибка',
			dirtyNote:
				'Статус относится к сохранённому тексту. Новый текст будет отправлен на одобрение после сохранения.',
			unavailable: 'Этот текст ещё не отправлялся оператору.',
			unavailableHint:
				'Возможно, SMS не настроены. Проверьте канал SMS в разделе «Настройки».',
		...
		outbox: {
			...
			retryFailed: 'Не удалось отправить повторно',
			sendFailed: 'Не удалось отправить сообщение',
			description: 'Статус, причина и стоимость каждого сообщения',
			empty: 'Сообщений пока нет.',
			loadError: 'Не удалось загрузить сообщения. Обновите страницу.',
			retry: 'Отправить повторно',
			retried: 'Сообщение снова в очереди',
			send: 'Отправить сообщение',
			sendTitle: 'Отправка сообщения вручную',
			sent: '{{count}} сообщений в очереди',
			sentWithSkipped:
				'{{count}} сообщений в очереди, {{skipped}} пропущено (нет номера)',
			column: {
				createdAt: 'Время',
				recipient: 'Кому',
				channel: 'Канал',
			...
			summary: {
				...
				deliveredHint: 'подтверждено оператором',
				failed: 'Ошибка',
				failedHint: 'можно повторить',
				inFlight: 'В процессе',
				inFlightHint: 'в очереди или отправляется',
				segments: 'Сегменты',
				segmentsHint: 'тарифицируемые части',
				deliveryRate: 'Доставляемость',
				deliveryRateHint: 'из завершённых',
			},
			range: {
				today: 'Сегодня',
				week: '7 дней',
			...
			pollingLive: '{{count}} в процессе · обновление',
			pollingIdle: 'Все завершены · опрос неактивен',
			retryAll: 'Повторить {{count}} с ошибкой',
			attemptShort: 'попытка {{count}}',
			sourceRule: 'Правило #{{id}}',
			sourceManual: 'Вручную',
			broadcastTemplate: 'Рассылка',
			errorLabel: 'Ошибка',
			...
			field: {
				channel: 'Канал',
				body: 'Текст сообщения',
				scope: 'Кому отправить',
				scopeBranch: 'По филиалу',
				scopeGroup: 'По группе',
				branch: 'Филиал',
				group: 'Группа',
				scheduledAt: 'Время отправки',
				scheduledAtHint: 'Если не указано, отправится сразу',
				bodyPlaceholder: 'Введите сообщение для отправки…',
			},
			compose: {
				description:
		...
		settings: {
			...
			description: 'Провайдер, имя отправителя и дневной лимит',
			saved: 'Настройки сохранены',
			loadError: 'Не удалось загрузить настройки. Обновите страницу.',
			masterSwitch: 'Включить отправку SMS',
			masterSwitchHint:
				'Главный переключатель для всех SMS в системе — правила, ручная отправка и повторные попытки останавливаются, когда он выключен.',
			notOperational:
				'SMS включены, но отправка невозможна — данные провайдера неполные.',
			balanceUnknown: 'Баланс неизвестен',
			channelCard: {
				title: 'Канал SMS',
				meta: 'Провайдер: {{provider}} · обновлено {{date}}',
				metaNoDate: 'Провайдер: {{provider}}',
			},
			field: {
				senderName: 'Имя отправителя',
				senderNameHint: 'Задаётся в панели провайдера, не здесь.',
				login: 'Логин провайдера',
				password: 'Секрет провайдера',
				credentialsHint:
					'Только для записи — оставьте оба поля пустыми, чтобы сохранить текущие данные. Секреты API никогда не возвращает.',
				dailyLimit: 'Дневной лимит (сообщений)',
			},
			credentials: {
				useOwn: 'Собственный аккаунт этого центра',
				useOwnHint:
					'Использовать собственный логин провайдера и утверждённое имя отправителя центра.',
				clear: 'Очистить сохранённые данные',
				cleared: 'Данные провайдера очищены',
				usingPlatform: 'Используется аккаунт провайдера платформы',
				usingOwn: 'Сохранены свои данные · {{login}}',
				moderationNote:
					'Одобрение оператора действует только для одного аккаунта. При переходе на собственный аккаунт все шаблоны отправляются на одобрение заново — до одобрения SMS не доставляются. Статус виден в разделе «Шаблоны».',
			},
			balance: {
				title: 'Баланс провайдера',
				fetched: 'обновлено {{date}}',
			},
			test: {
				title: 'Отправить тест',
				description:
					'Проходит через реального провайдера и оплачивается как обычное сообщение.',
				phone: 'Телефон',
				message: 'Сообщение (необязательно)',
				messagePlaceholder: 'По умолчанию — короткая проверка провайдера.',
				send: 'Отправить тест',
				success: 'Тестовое сообщение отправлено',
				failure: 'Не удалось отправить тест: {{error}}',
			},
			platformManaged: {
				title: 'Управляется Cohort',
				description:
					'Этот центр отправляет SMS через общий аккаунт Cohort. Баланс этого аккаунта, тестовые сообщения и модерация стандартных шаблонов управляются централизованно командой Cohort. Чтобы управлять этим самостоятельно, подключите выше собственный аккаунт Eskiz.',
			},
			validation: {
				credentialsPair: 'Укажите логин и пароль вместе',
			},
		},
		channel: {
			SMS: 'SMS',
		...
		audience: {
			STUDENT: 'Ученик',
			PRIMARY_GUARDIAN: 'Родитель',
			STUDENT_AND_GUARDIAN: 'Ученик и родитель',
			BILLING_CONTACTS: 'Руководство центра',
		},
		status: {
			QUEUED: 'В очереди',
