...
/** The headline value of a pay window — "1 200 000 / month" or "40% of fees". */
function configValueLabel(t: HrT, config: PayrollConfigResponse): string {
	return config.payrollType === 'PERCENT'
		? t('payroll.percentOfFees', { percent: config.payrollPercent ?? 0 })
		: t('payroll.perMonth', { amount: formatMoney(config.baseSalary ?? 0) });
}
		...
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				<Button
					...
					aria-label={t('payroll.actionsAria', {
						date: formatDate(config.effectiveFrom),
					})}
				>
...
export function PayrollConfigCard({ staffId }: { staffId: number }) {
	...
	function handleDeleteConfirm() {
		...
		deleteMutation.mutate(deletingConfig.id, {
			...
			onError: (err) => {
				toast.error(isApiError(err) ? err.message : t('payroll.deleteFailed'));
				setDeletingConfig(null);
			},
		});
	}
		...
		<Card>
			<CardContent className="flex flex-col gap-3">
				<div className="flex items-center justify-between">
					<p className="font-semibold">{t('payroll.cardTitle')}</p>
				...
				{isLoading ? (
					...
					<div className="flex flex-col gap-1.5">
						<StatusBadge kind="payroll_rate" status={current.payrollType}>
							{statusLabel('payroll_rate', current.payrollType)}
						</StatusBadge>
						<p className="text-xl font-bold tabular-nums">
							{configValueLabel(t, current)}
						</p>
			...
			<ConfirmDialog
				...
				description={
					hasPredecessor
						? t('payroll.confirmDelete.descriptionPrevious')
						: t('payroll.confirmDelete.descriptionNone')
				}
