...
- **The app bar carries no account controls.** Per the design, it holds the page title/subtitle, the bell, and — below `md` — the identity avatar. Language and sign-out live in the sidebar footer's dots menu (`OverflowMenu`), which is desktop-only; on a phone the **Profile screen** is the only route to them, which is why it carries the theme switch, the language select and the sign-out button. Keep sign-out reachable there even in Profile's loading and error states.
