...
interface SessionState {
	...
	subscription: SubscriptionAccessView | null;
	/**
	 * Set the instant any student request comes back 402 (`error.details`) — lets
	 * the block render immediately when a subscription lapses mid-session, without
...
/**
 * The student surface is gated by **role only** — it carries no permission codes, and
 * there is no `/student/me` boot fetch to resolve them from. So unlike the admin console,
 * this store holds no `permissions`: `user.roles` from the login/refresh response is the
 * whole authorization picture the client ever sees. It does track the tenant
 * `subscription`, which login/refresh reports for every surface.
 */
