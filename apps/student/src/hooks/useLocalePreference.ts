...
/**
 * Switches the UI language immediately (optimistic — local state is what the student
 * sees) and, when signed in, persists it to `PATCH /student/me` so the choice follows
 * them across sessions. A failed save is logged, never rolled back. Anonymous (login
