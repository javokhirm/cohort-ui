...
/**
 * Switches the UI language immediately (optimistic — local state is what the
 * teacher sees) and, when signed in, persists it to `PATCH /teach/me/preferences`
 * so the choice follows them to their phone and the admin console. A failed save
 * is logged, never rolled back. Anonymous (login screen) switches locally only.
 */
