...
- **The login endpoint is shared; this console is not.** `api/auth/auth.mutations.ts` sends `x-client-app: teacher` with the login call, and the backend refuses a non-TEACHER with 403 `CONSOLE_ROLE_NOT_ALLOWED` (`cohort-be/docs/api-reference.md` §1.1) — an OWNER/ADMIN/MANAGER can no longer authenticate here and land on a shell whose every request 403s. `useLogin()` keeps its own `RoleNotAllowedError` check as a fallback for a session that reaches the store without that server gate, and `requireRole(['TEACHER'])` guards the authed route for sessions restored via refresh.
